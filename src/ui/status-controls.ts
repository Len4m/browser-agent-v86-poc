...
function formatLoggedCommand(command, max = 360) {
  ...
  return `${text.slice(0, max)} … [${text.length} caracteres]`;
}
...
function syncDiskCheckButton() {
  ...
  button.textContent = state.diskMounted ? "Desmontar disco" : "Montar disco";
  button.title = state.diskMounted
    ? "Sincroniza y desmonta /mnt/hda. Fallará si algún proceso está usando el disco."
    : "Monta el disco hda en /mnt/hda y comprueba lectura/escritura.";
}
...
function syncPowerButtons() {
  ...
  if (powerButton) {
    ...
    powerButton.textContent = state.vmStarting ? "Arrancando…" : vmOn ? "Apagar VM" : "Arrancar VM";
    powerButton.title = vmOn
      ? "Apaga la VM. Se pedirá confirmación antes de perder cambios sin snapshot."
      : "Arranca la VM con la configuración actual.";
...
function syncWsButton() {
  ...
  button.textContent = state.wsConnecting ? "Conectando…" : connected ? "Desconectar" : "Conectar";
  button.classList.toggle("danger-light", connected);
  button.classList.toggle("secondary", connected);
  button.title = connected
    ? "Desconecta el proxy WebSocket local. Se pedirá confirmación."
    : "Conecta el proxy WebSocket local wsnic.";
}
...
function syncChecksButton() {
  ...
  button.textContent = state.checksRunning ? "Comprobando…" : (state.bgTools?.pending ? "Herramienta activa…" : "Comprobar");
...
function setAgentBusy(value, detail = "") {
  ...
  if (overlay) overlay.textContent = value ? (detail || "El agente está usando la VM…") : "";
