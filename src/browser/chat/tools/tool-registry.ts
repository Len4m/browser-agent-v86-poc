...
export const llmToolRegistry: LlmToolRegistryApi = (() => {
  ...
  function normalizeVmPath(value: unknown, fallback = "."): string {
  ...
  function captureCommand(prefix: unknown, requiredCommands: unknown, bodyCommand: string): string {
  ...
  function toolPrompt(action: string, args: string, extra = ""): string {
  ...
  function summaryHeadTarget(phraseKey: string, target: unknown): string {
  ...
  function summaryCouldNot(verbKey: string, target: unknown): string {
  ...
  function summaryToolOn(tool: string, target: unknown): string {
    return t("common.summaryToolOn", { tool, target: textValue(target) });
  }

  function summaryToolFailedOn(tool: string, target: unknown): string {
  ...
  function isToolEnabledForProfile(tool: ToolDefinition | undefined, profileId = baseRuntimeContext().activeProfile): boolean {
    ...
    const profile: VmProfile | undefined = runtimeProfile?.id === profileId
      ? runtimeProfile
      : state.profiles.find((item): item is VmProfile => isVmProfile(item) && item.id === profileId);
  ...
  function normalizeListArgs(args: ToolArgs = {}): ToolArgs { return { path: normalizeVmPath(args.path || "."), maxEntries: clampInt(args.maxEntries, 1, 300, 120) }; }
  function normalizeReadArgs(args: ToolArgs = {}): ToolArgs { return { path: normalizeVmPath(args.path || ""), maxBytes: clampInt(args.maxBytes, 256, 32768, 8192) }; }
  ...
  const TOOLS: Record<string, ToolDefinition> = {
    ...
    "vm.fs.write": {
      ...
      formatResult(result, args) { return standardFormat(this, result, args, () => t("tools.summary.fsWriteOk", { path: textValue(args.path) }), () => summaryCouldNot("common.verb.write", args.path)); },
    },
    ...
    "net.dns.lookup": {
      ...
      formatResult(result, args) { return standardFormat(this, result, args, () => t("common.summaryDns", { type: textValue(args.type), host: textValue(args.host) }), () => summaryCouldNot("common.verb.resolve", args.host)); },
    },
    ...
    "vm.sh.exec": {
      ...
      formatResult(result, args) {
        const oldMax = this.maxOutputBytes; this.maxOutputBytes = clampInt(args.maxOutputBytes, 512, 32768, 8192);
