...
(function initLLMAgentLoop() {
  ...
  function stopActiveTurn() {
    ...
    window.BA_LLM_EVENTS?.emit("status", { text: "Operación detenida", tone: "warn" });
  }
  ...
  async function ensureAiSdk() {
    ...
    if (!window.BA_AISDK) {
      throw new Error("AI SDK bridge no cargado. Ejecuta npm run build y recarga la página.");
    }
  ...
  async function loadSelectedModel() {
    ...
    if (!modelConfig.model) {
      throw new Error(modelConfig.engine === "ollama"
        ? "Indica un modelo de Ollama, por ejemplo qwen3.5:4b o qwen3:8b."
        : "Indica un modelo compatible con Transformers.js.");
    }
    if (needsWebGPU && !caps?.webgpu) {
      throw new Error(caps?.reason || "WebGPU no está disponible en este navegador. Selecciona un modelo experimental WASM o usa Chrome/Edge con WebGPU activado.");
    }
    if (needsWebGPU && modelRequiresUnavailableF16(modelConfig, caps)) {
      throw new Error(`El modelo seleccionado usa ${modelConfig.dtype}, pero esta GPU/navegador no soporta shader-f16. Selecciona una variante q4 o un modelo experimental WASM.`);
    }

    if (!window.BA_LLM_RESOURCE_GOVERNOR?.canStart?.("model-load")) {
      throw new Error("Hay una operación LLM/herramienta activa. Espera a que termine antes de cargar otro modelo.");
    }

    window.BA_LLM_RESOURCE_GOVERNOR?.start?.("model-load", "carga de modelo");
    llm.loading = true;
    llm.loaded = false;
    ...
    window.BA_LLM_EVENTS?.emit("status", {
      text: modelConfig.engine === "ollama" && window.BA_ORIGIN?.isPublishedOrigin?.()
        ? "Ollama local: permiso de red + OLLAMA_ORIGINS"
        : (modelConfig.engine === "ollama" ? "Conectando con Ollama…" : "Cargando modelo local…"),
      tone: "warn",
    });
      ...
      await sdk.loadModel(modelConfig, {
        onProgress(detail) {
          ...
          if (detail?.status === "fallback") {
            window.BA_LLM_EVENTS?.emit("status", { text: "WebGPU falló; probando alternativa WASM…", tone: "warn" });
          }
        },
      });
  ...
  async function handleToolUiAfterExecute({
    ...
    if (toolResult.cancelled) {
      const answer = "Herramienta cancelada por el usuario.";
    ...
    const tools = nativeToolsMode
      ? (window.BA_buildAiSdkTools?.({
        ...
        onToolStart({ toolCall }) {
          ...
          renderToolCallBubble(toolBubble, toolCall, "ejecutando…");
          toolBubbles.set(key, toolBubble);
          toolCall.__uiKey = key;
          setChatTailIndicator(`Ejecutando ${toolCall.tool || "tool"}…`);
        },
        async onToolEnd({ toolCall, toolResult, artifact }) {
          ...
          setChatTailIndicator("Generando respuesta final…");
        },
      }) || {})
      : {};
    ...
    try {
      ...
      const runnerOutput = await sdk.runAgentStreamTurn({
        ...
        onStreamPart(part) {
          ...
          if (textChunk) {
            ...
            setChatTailIndicator("Generando respuesta…");
      ...
      const finalText = streamFollowUp
        ...
        || (toolPhaseSeen && lastToolUi?.toolResult && !lastToolUi.toolResult.ok
          ? `No se pudo completar la tool: ${lastToolUi.toolResult.stderr || lastToolUi.toolResult.summary || "error"}`
          : "");
  ...
  async function handleUserMessage(userText) {
    const llm = window.BA_LLM;
    if (!isModelReady()) {
      addMessage("agent", "Carga primero un modelo real desde el panel LLM. Puede ser Transformers.js local o un modelo Ollama disponible en el host.");
      updateChatAvailability?.();
      return;
    }
    ...
    if (!window.BA_LLM_RESOURCE_GOVERNOR?.canStart?.("llm")) {
      addMessage("agent", "Hay una operación LLM/herramienta activa o la VM está ocupada. Espera a que termine antes de enviar otra petición.");
      updateChatAvailability?.();
      return;
    }
    ...
    updateChatAvailability?.();
    window.BA_LLM_EVENTS?.emit("status", { text: "Agente trabajando…", tone: "warn" });

    try {
      window.BA_LLM_EVENTS?.emit("status", { text: "Generando…", tone: "warn" });
      ...
      window.BA_LLM_EVENTS?.emit("status", { text: lastToolUi ? "Herramienta ejecutada" : "Modelo local listo", tone: "good" });
    } catch (error) {
      if (isAbortError(error) || isStaleTurn(turnGeneration)) {
        if (!stopRequested) {
          addMessage("agent", "Operación cancelada.");
          window.BA_LLM_EVENTS?.emit("status", { text: "Operación cancelada", tone: "warn" });
        }
        return;
      }
      ...
      const recovery = isRecoverableGpuMemoryError(message)
        ...
        : "";
      renderer.write(`⚠️ Error LLM/tools: ${message}${recovery}`);
      renderer.end();
      ...
      window.BA_LLM_EVENTS?.emit("status", { text: "Error LLM/tools", tone: "bad" });
    } finally {
  ...
  function clearHistory() {
    ...
    window.BA_LLM_EVENTS?.emit("status", { text: "Chat visible, historial LLM y artefactos limpiados", tone: "good" });
    updateChatAvailability?.();
  }
  ...
  function unloadModel() {
    ...
    updateChatAvailability?.();
    window.BA_LLM_EVENTS?.emit("status", { text: "modelo descargado", tone: "warn" });
  }
  ...
  function setChatSubmitStopMode(submit, isStop) {
    ...
    if (isStop) {
      ...
      submit.setAttribute("aria-label", "Detener generación o ejecución de tool");
      submit.title = "Detener (cancela inferencia o tool en curso)";
      submit.textContent = "Detener";
      submit.disabled = false;
      return;
    }
    submit.type = "submit";
    submit.classList.remove("is-stop");
    submit.setAttribute("aria-label", "Enviar mensaje");
    submit.title = "Enviar mensaje (Enter)";
  ...
  function updateChatAvailability() {
    ...
    if (input) {
      ...
      input.placeholder = canSend
        ? "Pregunta al modelo…"
        : (busy
          ? "Pulsa Detener para cancelar la operación en curso…"
          : (llm?.loaded ? "Espera a que termine la operación actual…" : "Carga primero un modelo en el panel LLM"));
