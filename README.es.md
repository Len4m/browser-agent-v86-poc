...
[![Versión beta](https://img.shields.io/badge/version-0.9.8--beta.11-orange)](https://github.com/Len4m/browser-agent-v86-poc)
...
Estado actual: **beta `0.9.8-beta.11`**. La versión `1.0.0` queda reservada para la primera publicación estable.
...
Requisitos principales: Node.js 26.8.1, pnpm 11.24.0, Linux (o macOS con herramientas de build compatibles con GNU), Docker, herramientas de sistema para generar initramfs/discos y conexión a Internet para descargar assets base, paquetes Alpine y los modelos de navegador cuando se cargan por primera vez. Las versiones quedan fijadas en `.nvmrc`, `.node-version` y `packageManager`.
...
No abras `index.html` como `file://`. `pnpm start` ya sirve `public/` con las cabeceras necesarias.
...
| [docs/schema-reference/](docs/schema-reference/README.md) | Referencia de campos del perfil VM generada desde JSON Schema (**en inglés**). |

Cada documento enlaza con su versión en el otro idioma. Para contribuir o desplegar el proyecto, usa `USAGE`, `ARCHITECTURE` y `VM_PROFILES_AND_TOOLS`. Para la semántica de los campos de perfiles VM, consulta `docs/schema-reference/`.

## Colaborar

Se agradece cualquier tipo de colaboración: [issues](https://github.com/Len4m/browser-agent-v86-poc/issues), informes de bugs, ideas, traducciones, mejoras de documentación, [pull requests](https://github.com/Len4m/browser-agent-v86-poc/pulls) y nuevos perfiles VM o tools de perfil. Puedes consultar las mejoras previstas y tareas pendientes en [TODO.md](TODO.md). Si quieres añadir capacidades al runtime, consulta la [guía de perfiles VM y tools](docs/VM_PROFILES_AND_TOOLS.es.md).
