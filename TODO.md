...
- [ ] Comparar **NE2000 (`ne2k`)** frente a **virtio-net** en v86 + Alpine: hoy la VM usa `net_device: { type: "ne2k" }` y el initramfs carga `ne2k-pci` (aunque ya empaqueta módulos `virtio*`). Implementar o conmutar a virtio, repetir escaneos y uso de red con wsnic (p. ej. `nmap`, `httpx`, `ffuf`, latencia, estabilidad DHCP) y valorar si virtio conviene como predeterminado en Alpine moderno.
