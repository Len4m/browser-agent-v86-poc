...
- [X] Resolver el papel de `validationCommands`: se eliminan del contrato de perfiles porque solo eran metadata impresa al final del build y no se ejecutaban en `setup`, `build`, `check`, runtime ni frontend. Las comprobaciones reales deben vivir en `buildCommands`, `firstBootCommands` o `runtimeChecks` de cada tool.
