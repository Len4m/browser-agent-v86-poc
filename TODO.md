...
- [X] Hacer que el botón **Comprobar** derive los checks de tools desde las propias definiciones de tool, no desde un mapa hardcodeado de paquetes en la UI. Opción propuesta: añadir `availabilityChecks`/`runtimeChecks` opcional a `ToolDefinition`, que cada tool declare los comandos mínimos que prueban su disponibilidad real (`command -v curl`, `command -v nikto.pl`, fallback de nombres para `httpx`, etc.) y que `checks-panel.ts` los obtenga desde el registry usando `allowedTools` del perfil activo. Mantener el check de paquetes instalado separado de los checks de disponibilidad de tools.
- [ ] Resolver el papel de `validationCommands`: ahora solo son metadata impresa al final del build y no se ven ni se ejecutan en runtime. Decidir si se eliminan, se renombran a checks de perfil estructurados (`profileRuntimeChecks`/`profileChecks` con `id`, `label`, `command`) o se mantienen solo como notas manuales mejor documentadas. Si se convierten en checks de perfil, usarlos para validaciones propias del perfil (wordlists, symlinks, ficheros generados por `buildCommands`), no para duplicar checks que pertenecen a cada tool.
