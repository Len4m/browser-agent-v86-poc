...
Both guest runners use Python 3. Every profile in `vm/profiles/*.json` must include the `python3` package; `pnpm test:all` and the profile builder fail if it is missing.
...
- Tool results are stored as artifacts in the **Resources and context** modal, available from the chat-header button: you can preview them, attach them to the next message, or delete them. Attachments respect the model's context budget and are omitted when there is not enough room.
...
| `pnpm test` | Runs fast module and lightweight integration tests |
| `pnpm test:e2e` | Runs real Chromium and v86 tests |
| `pnpm test:all` | Runs TypeScript, lint, fast tests, integrity checks, and E2E tests |
...
- **VM does not start**: run `pnpm prepare:local`, then `pnpm test:all`.
